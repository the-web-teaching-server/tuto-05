...
the `db_init.py` file and then performing actions on the DB in
...
2. Look now at the `models/user.py`. Two classes are defined:
...
3. Then, look at the `server.py` file.
   * Where do we inform `flask_login` it has to use `UserForLogin`?
   * Browse in your app to `/`. You should be redirected to `/login`.
     Find out the two elements in the code responsible for this behaviour.
...
4. Finally, open the developper tools in the browser (F12) and find
...
1. Edit the `/` handler and the corresponding template to
   ...
   **Note**: the users generally dislike when their email is disclosed to many people...
2. Deleting the cookies is currently the only way to logout from our website.
   What a mess! Create a route `/logout/` containing
   ```python3 flask_login.logout_user()```
...
3. We want the user to be able to register on our website.
   ...
   * Create two handlers `register_get/register_post` (take inspiration from
     ...
     before proceeding to the actual user creation (you should use the class
...
2. * Add a few posts in the `init_db.py` file (don't forget to create the table
   ...
   * Check that it correctly has inserted the data: enter
     ...
     SELECT * FROM posts;
     ```
3. Add a handler for the route `/post/` with method `POST`.
   This route will:
   ...
   * redirect to home.
4. Add a form in the `index.html` file with action set to
   `/post/` and method to `POST` to insert a post. Try it
   and check it has worked with `sqlite3 .data/db.sqlite`.
5. We now want to display all posts, with the date and
   the name of the author.
   **Be carefull:** fetch all the posts *and then* fetch the author's name for
   each post requires `1+n` SQL requests (where `n` is number of requests),
   instead of performing a single`JOIN` request to fetch the author's names
   with the posts.
...
6. Edit the handler for `/` and the `templates/index.html` template
   to actually display the list of posts.

## Optional part: Give it some style!
Make the application looks nice by using CSS here and there. For the forms
...
* [longer, need to use `Cmd`] check if the email is already used
  ...
  on the server). If so, display a message and recommand a new login instead.
...
* if you do not use a `onSubmit` message handler on a form, this one will be
  sent to the server in the same way it would have been sent in a "normal" HTML
  page.
